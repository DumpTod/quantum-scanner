...
def fetch_ohlcv(symbol: str, force: bool = False) -> pd.DataFrame | None:
    ...
    try:
        ...
        # Fyers free tier limits range — try 365 days first, fallback to 200, then 100
        for days in [365, 200, 100]:
            ...
            logger.info(f"{symbol}: {days}d fetch status={response.get('s')} candles={len(response.get('candles',[]))}")
            if response.get("s") == "ok" and len(response.get("candles", [])) >= BACKTEST_MIN_CANDLES:
                break
        else:
            logger.error(f"{symbol}: All date ranges failed. Last response: {response}")
            return None

        candles = response.get("candles", [])
        if len(candles) < BACKTEST_MIN_CANDLES:
            logger.warning(f"{symbol}: Only {len(candles)} candles after all attempts")
            return None
