...
def api_debug():
    ...
    try:
        t = get_token()
        results['token'] = t[:30] + '...' if t else 'EMPTY'
    except Exception as e:
        results['token'] = f'FAILED: {str(e)}'

    # Test RELIANCE with full error
    try:
        from fyers_apiv3 import fyersModel
        fyers = fyersModel.FyersModel(
            client_id=FYERS_CLIENT_ID,
        ...
        response = fyers.history(data=data)
        results['fyers_raw_response'] = response
    except Exception as e:
        results['fyers_error'] = str(e)

    # Test Supabase
    try:
        r = requests.get(f"{SUPABASE_URL}/rest/v1/fyers_tokens",
                         headers=SUPA_HEADERS,
                         params={"id":"eq.1","select":"*"},timeout=10)
