...
def cleanup_dupes():
    """Remove duplicate rows from backtest_trades keeping only latest per symbol/direction/entry_bar."""
    ...
    try:
        # Fetch all trades
        r = req.get(f"{SUPABASE_URL}/rest/v1/backtest_trades?select=id,symbol,direction,entry_bar",
        ...
        for dupe_id in dupes:
            req.delete(f"{SUPABASE_URL}/rest/v1/backtest_trades?id=eq.{dupe_id}",
                       headers=headers, timeout=10)
            deleted += 1

        return jsonify({"status": "ok", "total_rows": len(rows), "duplicates_removed": deleted})
    except Exception as e:
