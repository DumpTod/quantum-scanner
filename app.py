...
FYERS_TOTP_KEY  = "4OZK5LE3F3KAKHGXTEWSK3BAALXZDLRB"  # Long string from TOTP setup

# Already filled from your app creation:
...
def supa_get_cache(symbol, direction):
    """Return cached backtest summary or None."""
    try:
        r = requests.get(
            f"{SUPABASE_URL}/rest/v1/backtest_cache",
            headers=SUPA_HEADERS,
            params={"symbol": f"eq.{symbol}", "direction": f"eq.{direction}", "select": "*"},
            timeout=10
        )
...
def supa_save_cache(symbol, direction, bt):
    """Upsert backtest summary into backtest_cache."""
    try:
        ...
        requests.post(
            ...
            json={
                ...
                "expectancy": bt["expectancy"], "updated_at": datetime.utcnow().isoformat()
            },
            timeout=10
        )
    except Exception as e:
...
def supa_save_trades(symbol, direction, trades):
    """Delete old trades then insert new individual backtest trades."""
    if not trades:
        return
    try:
        # Delete old trades for this symbol+direction
        requests.delete(
        ...
        requests.post(
            ...
            json=[{
                ...
                "outcome": t["outcome"], "profit_pct": t["profit_pct"],
                "entry_bar": t["entry_bar"], "exit_bar": t["exit_bar"],
                "bars_held": t["bars_held"], "entry_date": t["entry_date"],
...
def generate_fyers_token():
    ...
    r1 = session.post("https://api-t2.fyers.in/vagator/v2/send_login_otp_v2",
                      json={"fy_id": FYERS_USER_ID, "app_id": "2"})
    if r1.status_code != 200:
        raise Exception(f"send_login_otp failed: {r1.text}")
    request_key = r1.json().get("request_key")

    r2 = session.post("https://api-t2.fyers.in/vagator/v2/verify_otp",
    ...
    r3 = session.post("https://api-t2.fyers.in/vagator/v2/verify_pin_v2",
                      json={"request_key": request_key2, "identity_type": "pin", "identifier": pin_hash})
    if r3.status_code != 200:
        raise Exception(f"verify_pin failed: {r3.text}")
    access_token_temp = r3.json().get("data", {}).get("access_token")

    r4 = session.post("https://api-t2.fyers.in/api/v3/token",
                      headers={"Authorization": f"Bearer {access_token_temp}"},
                      json={"fyers_id": FYERS_USER_ID, "app_id": FYERS_CLIENT_ID.split("-")[0],
                            "redirect_uri": FYERS_REDIRECT, "appType": "100",
                            "code_challenge": "", "state": "quantum", "scope": "",
    ...
    r5 = session.post("https://api-t2.fyers.in/api/v3/validate-authcode",
    ...
    print(f"[Fyers] Token generated at {datetime.now().strftime('%H:%M:%S')}")
...
def fyers_headers():
    return {"Authorization": f"{FYERS_CLIENT_ID}:{get_token()}", "Content-Type": "application/json"}
...
def fetch_stock_data(symbol, period="18mo"):
    try:
        ...
        r = requests.get(
            ...
            params={"symbol": f"NSE:{symbol}-EQ", "resolution": "D", "date_format": "1",
                    "range_from": date_from.strftime("%Y-%m-%d"),
                    "range_to":   date_to.strftime("%Y-%m-%d"), "cont_flag": "1"},
            timeout=15
        )
        if r.status_code != 200: return None
        data = r.json()
        if data.get("s") != "ok" or not data.get("candles"): return None
...
def backtest_quantum(df, direction):
    ...
    for i in range(60, len(df)-10, 5):
        ...
        for j in range(1, min(30,len(df)-i-1)+1):
            f=df.iloc[i+j]; exit_date=str(df.index[i+j])[:10]
            hit=None; exit_p=0.0
            if signal=="BUY":
            ...
            if hit:
                ...
                trades.append({"entry_price":round(ep,2),"stop_loss":round(sl,2),"target":round(tgt,2),
                                "exit_price":round(exit_p,2),"outcome":hit,"profit_pct":pnl,
                                "entry_bar":i,"exit_bar":i+j,"bars_held":j,
...
def quantum_scan(symbol):
    ...
    # ── Backtest: check Supabase cache first ──
...
def home():
    return jsonify({'scanner':'QUANTUM Scanner v2.0 (Fyers + Supabase)',
                    'endpoints':{'/api/scan':'Run full scan','/api/scan?max=50':'Quick scan',
                                 '/api/results':'Cached results','/api/health':'Health check',
                                 '/api/token':'Force token refresh',
                                 '/api/backtest/<symbol>/<direction>':'Get all backtest trades'}})
...
def api_backtest_trades(symbol, direction):
    """Fetch all individual backtest trades for a symbol from Supabase."""
    try:
        r=requests.get(f"{SUPABASE_URL}/rest/v1/backtest_trades",
                       headers=SUPA_HEADERS,
                       params={"symbol":f"eq.{symbol.upper()}",
                               "direction":f"eq.{direction.upper()}","select":"*"},
                       timeout=10)
        if r.status_code==200:
