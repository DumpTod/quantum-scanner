...
# SHA256 hash of client_id:secret_key (never changes)
...
# ============================================================
# TOKEN MANAGER — reads from Supabase, refreshes daily
# ============================================================

_access_token  = None
_token_date    = None   # date when token was last generated
...
def save_access_token(access_token):
    """Save freshly generated access_token back to Supabase."""
    try:
        requests.patch(
            f"{SUPABASE_URL}/rest/v1/fyers_tokens",
            headers=SUPA_HEADERS,
            params={"id": "eq.1"},
            json={"access_token": access_token, "updated_at": datetime.utcnow().isoformat()},
            timeout=10
        )
    except Exception as e:
...
def refresh_access_token(refresh_token):
    """Use refresh_token to generate a new access_token (valid 1 day)."""
    try:
        r = requests.post(
        ...
        if r.status_code == 200:
            ...
            if token:
                print(f"[Fyers] Access token refreshed at {datetime.now().strftime('%H:%M:%S')}")
                return token
            print(f"[Fyers] Refresh response: {data}")
    except Exception as e:
        print(f"[Fyers] refresh_access_token error: {e}")
    return None

def get_token():
    """Return valid access token. Refreshes once per day automatically."""
    ...
    if not tokens:
        raise Exception("No tokens found in Supabase. Run Colab notebook first.")

    refresh_token = tokens.get("refresh_token")
    if not refresh_token:
        raise Exception("refresh_token missing in Supabase. Run Colab notebook.")
    ...
    # Fallback: use stored access_token from Supabase (same day)
    stored = tokens.get("access_token")
    if stored:
        print("[Fyers] Using stored access token from Supabase.")
...
# ============================================================
# SUPABASE — Backtest cache helpers
# ============================================================
...
def fetch_stock_data(symbol, period="18mo"):
    try:
        ...
        r = requests.get(
            ...
            params={"symbol": f"NSE:{symbol}-EQ", "resolution": "D",
                    "date_format": "1",
...
FNO_STOCKS = {
    ...
    "TATACHEM","TATACOMM","TATACONSUM","TATAELXSI","TMCV","TMPV","TATAPOWER","TATASTEEL",
...
@app.route('/api/token')
def api_token():
    """Force a fresh token refresh from Supabase refresh_token."""
    global _access_token, _token_date
    _access_token=None; _token_date=None   # clear cache to force refresh
    try:
        get_token()
